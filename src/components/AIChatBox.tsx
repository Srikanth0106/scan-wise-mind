...
export function AIChatBox({ resumeId, resumeText, candidateName }: AIChatBoxProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const resp = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat-resume`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="flex flex-col h-full bg-card border border-border rounded-lg dark:card-glow dark:border-glow overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
        {messages.length === 0 && (
          <div className="text-center text-muted-foreground py-8">
            ...
            <p className="text-sm">Ask me anything about {candidateName}'s resume.</p>
            <div className="mt-3 flex flex-wrap gap-2 justify-center">
              {["Summarize skills", "Years of experience?", "Strengths & weaknesses?"].map(q => (
                <button
                  key={q}
                  onClick={() => { setInput(q); }}
